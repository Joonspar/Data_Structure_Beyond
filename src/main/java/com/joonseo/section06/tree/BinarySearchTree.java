...
/* 이진 트리
*  각 노드가 최대 2개의 자식을 가지며 왼쪽 자식과 오른쪽 자식으로 구분된다.
*
*  이진 탐색 트리
*  이진 트리의 구조를 가지면서 추가로 왼쪽 서브트리의 모든 노드는 현재 노드 값보다 작고,
*  오른쪽 서브 트리의 모든 노드는 현재 노드 값보다 크다는 규칙을 만족한다.
*
...
public class BinarySearchTree<T extends Comparable<T>> {
    ...
    private Node<T> insertRec(Node<T> node, T data){
        if(node == null){ // 아무거도 없을 때 삽입하는 기능
            return new Node<>(data);
        }
        // data가 현재 노드의 data 보다 작으면 => 왼쪽 서브트리에 삽입
        if(data.compareTo(node.data) < 0){ // 컬렉션에서 갖고옴
            node.left = insertRec(node.left,data);
        }
        // data가 현재 노드의 data보다 크면 => 오른쪽 서브트리에 삽입
        else if(data.compareTo(node.data) > 0){ // 컬렉션에서 갖고옴
    ...
    private void inOrderRec(Node<T> node, List<T> result){ // 딱 정렬된 순서대로 데이터가 반환된다.
        if(node != null){
    ...
    private void postOrderRec(Node<T> node, List<T> result){ // 트리를 삭제할 때 사용
        if(node != null){
